    ...
    public partial class ProjectileSpawnSystem : SystemBase
    {
                    ...
                    in ProjectileSettings settings) =>
                {
                    ...
                    else
                    {
                        ...
                        if (isInBattleGround)
                        {
                            ...
                            SpawnProjectile(entityInQueryIndex, ref beginSimulationCommandBuffer,
            ...
            in ProjectileSettings settings)
        {
            ...
            beginSimulationCommandBuffer.SetComponent(
                entityInQueryIndex, projectile, new Translation { Value = worldPosition });

            beginSimulationCommandBuffer.SetComponent(
                entityInQueryIndex, projectile, new Rotation { Value = worldRotation });

            beginSimulationCommandBuffer.AddComponent<ProjectileStartPosition>(entityInQueryIndex, projectile);

            beginSimulationCommandBuffer.SetComponent(
                entityInQueryIndex, projectile, new ProjectileStartPosition { Value = worldPosition });

            beginSimulationCommandBuffer.AddComponent<ProjectileSpeed>(entityInQueryIndex, projectile);

            beginSimulationCommandBuffer.SetComponent(
                entityInQueryIndex, projectile, new ProjectileSpeed { Value = settings.Speed });

            beginSimulationCommandBuffer.AddComponent<ProjectileDestroyDistance>(entityInQueryIndex, projectile);

            beginSimulationCommandBuffer.SetComponent(
                entityInQueryIndex, projectile, new ProjectileDestroyDistance { Value = settings.DestroyDistance });
        }
    }
}